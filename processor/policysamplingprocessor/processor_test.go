...
func TestPolicyEvaluator(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			if tC.drop {
				assert.Empty(t, next.Traces)
			} else {
				assert.Len(t, next.Traces, 1)
			}
		})
	}
}
...
func TestProcessTraces(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			// verify
			if tC.drop {
				assert.Empty(t, tC.next.Traces)
			} else {
				assert.Len(t, tC.next.Traces, 1)
			}
		})
	}
}
...
func TestCapabilities(t *testing.T) {
	// prepare
	processor, err := newTraceProcessor(nil, &componenttest.ExampleExporterConsumer{}, Config{})
	require.NotNil(t, processor)
	require.NoError(t, err)
...
func (m *mockNextConsumer) ConsumeTraces(ctx context.Context, td pdata.Traces) error {
	m.ExampleExporterConsumer.ConsumeTraces(ctx, td) // record the trace in the mock
	if m.failOnConsume {
