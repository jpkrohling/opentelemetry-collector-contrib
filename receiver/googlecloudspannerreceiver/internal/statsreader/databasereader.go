...
func NewDatabaseReader(ctx context.Context,
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Error occurred during client instantiation for database %q", databaseID.ID()))
		return nil, err
	}
...
func (databaseReader *DatabaseReader) Shutdown() {
	databaseReader.logger.Info(fmt.Sprintf("Closing connection to database %q", databaseReader.database.DatabaseID().ID()))
	databaseReader.database.Client().Close()
}

func (databaseReader *DatabaseReader) Read(ctx context.Context) []pdata.Metrics {
	databaseReader.logger.Info(fmt.Sprintf("Executing read method for database %q", databaseReader.database.DatabaseID().ID()))

	var result []pdata.Metrics

	for _, reader := range databaseReader.readers {
		if metrics, err := reader.Read(ctx); err != nil {
			databaseReader.logger.Error(fmt.Sprintf("Cannot read data for metrics databaseReader %q because of error %v",
				reader.Name(), err))
		} else {
